...
def main(config, startRecID, endRecID, filterIndicator, value, verbose):
    ...
    latent_space_calculator = LatentSpaceCalculator(config['transformer_config'], verbose=verbose)
    ...
    for numRec, idRecord in enumerate(recordIDlist):
        ...
        conflict_df['LOF1'] = np.nan
        conflict_df['LOF2'] = np.nan
        conflict_df['is_anomaly1'] = np.nan
        conflict_df['is_anomaly2'] = np.nan
    ...
    for (cluster_a, cluster_b), grp in filtered_conflicts.groupby('pair_key'):
        ...
        t0_comb = time.perf_counter()  # <-- start timing this combination
        num_conf_in_grp = int(len(grp))
        ...
        for rule_option in rule_options:
            ...
            for cluster in [cluster_a, cluster_b]:
                ...
                for vehicle_class in vehicle_classes:
                    ...
                    title_embeddings = "Embeddings_" + title + ".parquet"
        ...
        for numRec, idRecord in enumerate(recordIDlist):
            ...
            for index, row_rec in record_df.iterrows():
                ...
                if mask.any():
                    conflict_df.loc[mask, 'LOF1'] = row_rec['LOF']
                    conflict_df.loc[mask, 'is_anomaly1'] = row_rec['is_anomaly']
                ...
                if mask.any():
                    conflict_df.loc[mask, 'LOF2'] = row_rec['LOF']
                    conflict_df.loc[mask, 'is_anomaly2'] = row_rec['is_anomaly']

            # Save conflict_df
            conflict_df.to_csv(conflict_file, sep=',', index=False)

        # --- end timing for this combination ---
        dt_comb = time.perf_counter() - t0_comb
        timing_rows.append({
            "cluster_comb": f"{cluster_a}_{cluster_b}",
            "num_conflicts": num_conf_in_grp,
            "time": dt_comb
        })

    if timing_rows:
        total_time = sum(r["time"] for r in timing_rows)
        total_conflicts = sum(r["num_conflicts"] for r in timing_rows)
        total_combs = len(timing_rows)

        # last row = totals (as requested)
        timing_rows.append({
            "cluster_comb": total_combs,  # number of all combinations
            "num_conflicts": total_conflicts,  # total conflicts across all combinations
            "time": total_time  # total elapsed time
        })

        df_time = pd.DataFrame(timing_rows, columns=["cluster_comb", "num_conflicts", "time"])

        ts = datetime.now().strftime("%Y%m%d%H%M%S")
        cfg_base = os.path.splitext(os.path.basename(args.config))[0]
        out_name = f"{ts}_time_measurement_lof_calc_transformer_{cfg_base}.csv"
        out_path = os.path.join(config['output_config']['output_folder'], out_name)

        df_time.to_csv(out_path, index=False)
        print(f"[TIME] Saved per-combination timing CSV to {out_path}")
